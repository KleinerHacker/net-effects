...
float4 inColor : register(c1);
float opacity : register(c2);
bool change : register(c0);
int blendType : register(c3);

float4 main(float2 uv : TEXCOORD) : COLOR
{
    float4 color1 = change ? inColor : tex2D(implicitInput, uv);
	float4 color2 = change ? tex2D(implicitInput, uv) : inColor;
